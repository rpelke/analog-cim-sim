...
TEST(INTLibTests, TNN_IV_split) {
    ...
    for (bool d : digital) {
        std::string cfg =
            get_cfg_file(digital_to_foldername(d) + "TNN_IV_split.json");
        set_config(cfg.c_str());
        int32_t status = cpy_mtrx(mat, m_matrix, n_matrix);
        ASSERT_EQ(status, 0) << "Matrix write operation failed.";
...
TEST(INTLibTests, TNN_V_split) {
    ...
    for (bool d : digital) {
        std::string cfg =
            get_cfg_file(digital_to_foldername(d) + "TNN_V_split.json");
        set_config(cfg.c_str());
        int32_t status = cpy_mtrx(mat, m_matrix, n_matrix);
        ASSERT_EQ(status, 0) << "Matrix write operation failed.";

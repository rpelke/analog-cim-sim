...
CrossbarMapping::CrossbarMapping(
    uint32_t M, uint32_t N, uint32_t W_BIT, uint32_t I_BIT,
    std::vector<uint32_t> SPLIT, const bool digital_only,
    const INT8MappingMode m_mode, const float HRS, const float LRS,
    const ADCType adc_type, const float min_adc_curr, const float max_adc_curr,
    const float alpha, const uint32_t resolution, const bool verbose) :
    d_xbar_(M, N, W_BIT, I_BIT, SPLIT, digital_only, m_mode),
    a_xbar_(M, N, W_BIT, I_BIT, SPLIT, HRS, LRS, m_mode, adc_type, min_adc_curr,
            max_adc_curr, alpha, resolution),
    i_bit_(I_BIT), split_(SPLIT), digital_only_(digital_only), m_mode_(m_mode),
    write_xbar_counter_(0), mvm_counter_(0), verbose_(verbose) {}
...
CrossbarMapping::~CrossbarMapping() {
    if (verbose_) {
        std::cout << "INT8MappingMode: " << m_mode_to_string(m_mode_)
                  << std::endl;

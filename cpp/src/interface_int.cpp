...
extern "C" int32_t exe_mvm(pybind11::array_t<int32_t> res,
...
extern "C" int32_t cpy_mtrx(pybind11::array_t<int32_t> mat, int32_t m_matrix,
                            int32_t n_matrix) {
    if (xbar == nullptr) {
        std::cerr << "Error: Crossbar is not initialized. Please call "
                     "set_config() first."
    ...
    auto mat_buffer = mat.request();
    int32_t *mat_ptr = static_cast<int32_t *>(mat_buffer.ptr);
    xbar->write(mat_ptr, m_matrix, n_matrix);
    return 0;
}

PYBIND11_MODULE(acs_int, m) {
    m.def("cpy_mtrx", &cpy_mtrx, "Copy matrix to crossbar.");
    m.def("exe_mvm", &exe_mvm, "Execute matrix-vector multiplication.");

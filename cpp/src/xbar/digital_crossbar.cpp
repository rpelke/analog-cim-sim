...
DigitalCrossbar::DigitalCrossbar(uint32_t M, uint32_t N, uint32_t W_BIT,
                                 uint32_t I_BIT, std::vector<uint32_t> SPLIT,
                                 const bool digital_only,
                                 const INT8MappingMode m_mode) :
    m_(M),
    n_(N), w_bit_(W_BIT), i_bit_(I_BIT), split_(SPLIT),
    gd_p_(m_ * split_.size(), std::vector<int32_t>(n_, 0)),
    gd_m_(m_ * split_.size(), std::vector<int32_t>(n_, 0)),
    shift_(SPLIT.size(), 0), digital_only_(digital_only), m_mode_(m_mode),
    ct_constants_(M, 0) {
...
void DigitalCrossbar::write(const int32_t *mat, int32_t m_matrix,
                            int32_t n_matrix) {
...
void DigitalCrossbar::mvm(int32_t *res, const int32_t *vec, const int32_t *mat,
...
int DigitalCrossbar::get_m_xbar() const { return gd_p_.size(); }

int DigitalCrossbar::get_n_xbar() const { return gd_p_[0].size(); }

const std::vector<int32_t> &DigitalCrossbar::get_ct_constants() const {
    return ct_constants_;
}

void DigitalCrossbar::mvm_i_diff_w_diff(int32_t *res, const int32_t *vec,
                                        const int32_t *mat, int32_t m_matrix,
                                        int32_t n_matrix) {
    // The splitted matrix is of size split_size*M x N (split_size values per
    // original matrix value) Two matrices exist: gd+ (gd_p_) and gd- (gd_m_)
...
void DigitalCrossbar::mvm_i_offs_w_diff(int32_t *res, const int32_t *vec,
                                        const int32_t *mat, int32_t m_matrix,
                                        int32_t n_matrix,
                                        const std::vector<int32_t> *ct_const) {
    // The splitted matrix is of size split_size*M x N (split_size values per
...
void DigitalCrossbar::mvm_i_tc_w_diff(int32_t *res, const int32_t *vec,
                                      const int32_t *mat, int32_t m_matrix,
                                      int32_t n_matrix) {
    // The splitted matrix is of size split_size*M x N (split_size values per
    // original matrix value) Two matrices exist: gd+ (gd_p_) and gd- (gd_m_) In
    // this case, the input values (which are signed) are interpreted as two's
    // complement
    const uint32_t tmp_size = m_matrix * split_.size();
    std::vector<int32_t> tmp_out(tmp_size, 0);
    std::vector<int32_t> vd_p(n_matrix, 0);
...
void DigitalCrossbar::mvm_i_uint_w_diff(int32_t *res, const int32_t *vec,
                                        const int32_t *mat, int32_t m_matrix,
                                        int32_t n_matrix) {
    // The splitted matrix is of size split_size*M x N (split_size values per
    // original matrix value) Two matrices exist: gd+ (gd_p_) and gd- (gd_m_) In
    // this case, the input values 'vec' are stored in int32_t but they are all
    // positive
    const uint32_t tmp_size = m_matrix * split_.size();
    std::vector<int32_t> tmp_out(tmp_size, 0);
...
void DigitalCrossbar::mvm_i_uint_w_offs(int32_t *res, const int32_t *vec,
                                        const int32_t *mat, int32_t m_matrix,
                                        int32_t n_matrix) {
    // The splitted matrix is of size split_size*M x N (split_size values per
    // original matrix value) Only one matrix exist: gd+ (gd_p_) The input

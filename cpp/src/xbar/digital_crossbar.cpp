        ...
        for (size_t m = 0; m < m_matrix; ++m) {
            for (size_t n = 0; n < n_matrix; ++n) {
                ...
                for (size_t s = 0; s < CFG.SPLIT.size(); ++s) {
                    int gd_idx = m * CFG.SPLIT.size() + s;
                    if (mat_val >= 0) {
                        gd_p_[gd_idx][n] =
                            (mat_val >> shift_[s]) & ((1 << CFG.SPLIT[s]) - 1);
                        gd_m_[gd_idx][n] = 0;
                    } else {
                        gd_p_[gd_idx][n] = 0;
                        gd_m_[gd_idx][n] =
...
void DigitalCrossbar::mvm_i_diff_w_diff(int32_t *res, const int32_t *vec,
    ...
    for (size_t m = 0; m < m_matrix; ++m) {
        for (size_t s = 0; s < CFG.SPLIT.size(); ++s) {
            res[m] += tmp_out[m * CFG.SPLIT.size() + s] << shift_[s];
        }
    }

...
AnalogCrossbar::AnalogCrossbar(uint32_t M, uint32_t N, uint32_t W_BIT,
                               uint32_t I_BIT, std::vector<uint32_t> SPLIT,
                               const float HRS, const float LRS,
                               const INT8MappingMode m_mode,
                               const ADCType adc_type, const float min_adc_curr,
                               const float max_adc_curr, const float alpha,
...
void AnalogCrossbar::mvm_i_diff_w_diff(int32_t *res, const int32_t *vec,
    ...
    // original matrix value) Two matrices exist: ia+ (ia_p_) and ia- (ia_m_).
    ...
    // ia_m_ For positive inputs vd_p: bit 7 is always 0 (sign bit) -> i_bit_ -
...
void AnalogCrossbar::mvm_i_offs_w_diff(int32_t *res, const int32_t *vec,
    ...
    // For each bit in vd_p execute one MVM operation with ia_p_ and one with
    // ia_m_ MSB of input has position: i_bit_ + 1 Subract both results in the
    // analog domain
    for (size_t i_bit = 0; i_bit < i_bit_ + 1; ++i_bit) {
        // Calculcate multiplications with negative and positive weights
        for (size_t t_m = 0; t_m < tmp_size; ++t_m) {
...
void AnalogCrossbar::mvm_i_uint_w_offs(int32_t *res, const int32_t *vec,
    ...
    // The splitted matrix is of size split_size*M x N (split_size values per
    // original matrix value) Only one matrix exist: ia+ (ia_p_) The input is
